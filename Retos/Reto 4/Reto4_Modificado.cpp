...
unordered_map<string, int> conexionesPorDia(string fecha) 
{
    ...
    for( Record r :data)
    {
        // Evaluar que el record sea de la fecha solicitada, el nombreDestino no sea "-", que no contenga reto.com
        if((dia == getDay(r.fecha)) && (r.nombreDestino != "-") && (r.nombreDestino.find("reto.com") == string::npos))
        {
            // Si el dominio / nombre destino aun no se encuentra en el diccionario
            if(cnxPorDiaDict.find(r.nombreDestino) == cnxPorDiaDict.end())
                cnxPorDiaDict[r.nombreDestino] = 1; // se agrega el dominio al diccionario, inicializado con 1 conexion (la de la conexion entrante del record actual)
            
            else
                cnxPorDiaDict[r.nombreDestino]++; // se incrementa el numero de conexiones (cada vez que un dominio aparezca como destino, significa que tiene 1 conexion entrante)
        }
    }
